...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (intval($response_keys["success"]) !== 1) {
        ...
        $stmt = $conn->prepare("SELECT id, username, password, position FROM users WHERE username = ?");
        $stmt->bind_param("s", $username);
        $stmt->execute();
        $stmt->store_result();

        if ($stmt->num_rows > 0) {
            $stmt->bind_result($guest_id, $name, $hashed_password, $role);
            $stmt->fetch();
                ...
                <div class="col-md-6">
                    <form action="" method="post">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" id="form1Example13" class="form-control py-3" name="username" autocomplete="off" placeholder="Enter your username" required>
                        </div>
                        <div class="mb-4">
